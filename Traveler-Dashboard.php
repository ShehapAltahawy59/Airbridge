...
$request_stmt = $pdo->prepare("SELECT * FROM deliveryrequests WHERE Traveler_id = :user_id");
...
if (isset($_POST['delete_request'])) {
  ...
  $fetch_stmt = $pdo->prepare("SELECT SenderID FROM deliveryrequests WHERE RequestID = :request_id AND Traveler_id = :user_id");
  ...
  $delete_stmt = $pdo->prepare("UPDATE deliveryrequests SET Status = 'Pending', Traveler_id = NULL WHERE RequestID = :id");
  $delete_stmt->bindParam(':id', $request_id, PDO::PARAM_INT);
  $delete_stmt->execute();

  // If TravelerID is not null, send a notification

  $sender_id = $request_data['SenderID'];
  $message = "The request (ID: $request_id)  has been canceled by the traveller.";

  // Insert notification into the notifications table
  $notification_stmt = $pdo->prepare("INSERT INTO notifications (user_id, body) VALUES (:sender_id, :message)");
  $notification_stmt->bindParam(':sender_id', $sender_id, PDO::PARAM_INT);
...
<body>
  ...
  <section class="container">
    <h1 class="dashboard-header">Traveller Dashboard</h1>
    <div class="dashboard-buttons">
      <a href="TravellerRequestList.php" class="btn btn-custom">Requests List</a>
      <a href="NotificationList.php" class="btn btn-custom">Notifications</a>
    </div>
            ...
            <tr>
              ...
              <td>
                <form method="POST" style="display:inline;">
                  ...
                  <button type="submit" name="delete_request" class="btn btn-danger btn-sm">Cancel</button>
                </form>
